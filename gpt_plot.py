...
def visualize_tensors(coordinates, values, name=None, plot_type='scatter', marker_symbol='circle', marker_size=2, marker_color='red', fig=None, mode='markers', subplot=[1, 1]):
    ...
    elif plot_type.lower() == 'gmt':
        # assumes square coordinate field
        len = int(coordinates.size(0)**(1/2))
        values_xr = xr.DataArray(values.reshape(len, len))
        fig = gmt_surface(values_xr)
    else:
